...
public static class ArchitectureTypeFactory
{
    ...
    /// </summary>
    private static readonly Lazy<Dictionary<string, Type>> _architectureTypes = new(LoadArchitectureTypes);

    /// <summary>
    /// Gets a dictionary of all available architecture types, keyed by their lowercase names.
    /// </summary>
    public static IReadOnlyDictionary<string, Type> AvailableArchitectureTypes => _architectureTypes.Value;
    ...
    public static ArchitectureType Create(string name)
    {
        ...
        if (!_architectureTypes.Value.TryGetValue(name.ToLowerInvariant(), out var type))
