...
public class NewDddCommand: NewCommand
{
    public NewDddCommand() : base("ddd", "Creates a new DDD project")
    {
        ...
        this.SetHandler(HandleDddCommand, outputOption, nameOption, apiTypeOption, dbTypeOption, applicationOption, domainOption);
    }
    
    private void HandleDddCommand(string output, string? name, string api, string? db, string? application, string? domain)
    {
